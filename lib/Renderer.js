...
class Renderer extends EventEmitter
{
    ...
    async render(viewScript, viewData = {}) {
        ...
        if(options.viewPath) { // if addition view path provided, we will simply add to the options
            options.viewPaths.push(options.viewPath);
        }
    ...
    content(script, options) {
        ...
        let {viewPaths, viewExt, viewEngines} = options;
        let viewEngine = this.engine(script, options);

        let filename = this.resolve(script, viewPaths, viewExt);
        return cons[viewEngine](filename, options);
    }
    ...
    engine(script, options) {
        ...
        return options.viewEngines[ext];
    }
